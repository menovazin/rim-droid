 ...
 */
class LocationPagingSourceTest : BaseTest() {

    private val api: RickAndMortyApi = mock()
    private lateinit var source: LocationPagingSource

    @Before
    fun setUp() {
        source = LocationPagingSource(api)
    }

    @Test
    fun `load refresh first page returns nextKey 2`() = runUnconfinedTest {
        whenever(api.getLocations(any())).thenReturn(pageOne())
    ...
    fun `load append advances keys`() = runUnconfinedTest {
        whenever(api.getLocations(any())).thenReturn(pageTwo())
    ...
    fun `load returns null nextKey at end of pagination`() = runUnconfinedTest {
        whenever(api.getLocations(any())).thenReturn(lastPage())
    ...
    fun `load returns error when api throws`() = runUnconfinedTest {
        ...
        whenever(api.getLocations(any())).thenThrow(thrown)
    ...
    fun `getRefreshKey returns null without anchor`() {
        val state = PagingState<Int, Location>(
            pages = emptyList(),
            anchorPosition = null,
            config = PagingConfig(pageSize = 10),
            leadingPlaceholderCount = 0,
    ...
    private companion object {
        ...
        fun pageOne() = PagedResponseDto(
            ...
            results = listOf(
                LocationDto(id = 1, name = "Earth"),
                LocationDto(id = 2, name = "Abadango"),
            ),
        )

        fun pageTwo() = PagedResponseDto(
            info = InfoDto(pages = 10, next = "${BuildConfig.BASE_URL}location?page=6"),
            results = listOf(LocationDto(id = 11, name = "Citadel of Ricks")),
        )

        fun lastPage() = PagedResponseDto(
            info = InfoDto(pages = 1, next = null),
            results = listOf(LocationDto(id = 1, name = "Earth")),
        )

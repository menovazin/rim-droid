...
import com.smarttoolfactory.zoom.util.getNextZoomLevel
        ...
        val transformModifier = Modifier.pointerInput(key) {
            ...
            detectTransformGestures(
                ...
                onGesture = { centroid, pan, zoom, rotate, mainPointer, pointerList ->
                    ...
                    // RIM fork: pan/consume/zoom only with 2+ pointers so one-finger scroll parents work.
                    val multiTouch = pointerList.size >= 2
                    val gestureEnabled =
                        multiTouch && enabled(currentZoom, currentPan, currentRotation)

                    coroutineScope.launch {
                        enhancedZoomState.onGesture(
                            centroid = centroid,
                            pan = if (gestureEnabled) pan else Offset.Zero,
                            zoom = if (multiTouch) zoom else 1f,
                            rotation = if (multiTouch) rotate else 0f,
